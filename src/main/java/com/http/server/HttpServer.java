...
public class HttpServer {
    ...
    public void start() {
        if (running) {
            ConsoleWriter.logServer("服务器已经在运行中");
            return;
        }
        running = true;
        //启动服务器，监听端口，处理连接
        ...
        try {
            ...
            while (running) {
                try {
                    ...
                    if (running) {
                        ConsoleWriter.logError("连接处理异常: " + e.getMessage());
                    }
                }
            }
        } catch (IOException e) {
    ...
    public void stop() {
        ...
        try {
            ...
            ConsoleWriter.logServer("🛑 HTTP服务器已停止");
        } catch (IOException e) {
            ConsoleWriter.logError("停止服务器时发生错误: " + e.getMessage());
        }
    }
